...
#defining a class InputEmbeddings that inherits from nn.Module
class InputEmbeddings(nn.Module):

    def __init__(self, d_model: int, vocab_size: int):
        # d_model -> size of each input embedding (e.g., 512 in Transformer)
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len : int, dropout : int) -> None:
        ...
        # a vector of shape (seq_len, 1) to track position of words in seq
        position = torch.arrange(0, seq_len, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(torch.arrange(0, d_model, 2).float() * (-math.log(10000.0) / d_model))
        # sin to even postions and cos to odd positions 
        ...
        x_unsqueezed = x.unsqueeze(0)   # Add a new dimension at axis=0
        print(x.shape)         # torch.Size([4])
        print(x_unsqueezed.shape)  # torch.Size([1, 4])
        '''
...
class LayerNormalization(nn.Module):
    def __init__(self, eps: float = 10**-6) -> None:
        super().__init__()
        self.eps = eps
        #Using nn.parameter to make this parameter learnable
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        if mask is not None:
            attention_scores.masked_fill_(mask==0, -1e9) # If a position is masked, we set its attention score to -∞ before softmax.
        # This ensures softmax assigns zero probability to masked positions. Sentences in a batch may have different lengths, so we pad shorter ones with <PAD> tokens. We mask out padding tokens so they do not affect attention calculations.
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x , encoder_output, src_mask , tgt_mask): 
        # src mask is the mask of the input coming from the encoder  while tgt mask is the mask of the input coming from the decoder
...
class Transformer(nn.Module):
    ...
    def encode(self, src, src_mask):
        ...
        return self.ecoder(src, src_mask)
